    ...
    override fun register() {
        CommandBuilder()
                ...
                .setExecutor {
                    ...
                    if(newPosition == mm.queue.size) newPosition -= 1
                    if(current == 0) current += 1

                    if(current >= mm.queue.size || newPosition > mm.queue.size || current == 0) {
                        channel.sendEmbed("Music", "Invalid song position", Colors.RED)
                        return@setExecutor
                    }

                    val track = mm.queue.removeAt(current)
                    mm.queue.add(newPosition, track)
